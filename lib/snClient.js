    ...
    const getUpdateSetDetails = (updateSetSysId) => {
        return client.get({
            ...
            qs: {
                ...
                sysparm_fields: 'sys_id, application.name, application.scope, application.sys_id, name, description, state, remote_sys_id, sys_created_by, sys_created_on, sys_updated_by, sys_updated_on'
            }
        }).then((result) => {
            if (!result.length)
