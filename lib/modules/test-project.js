...
module.exports = function ({ id, commitId, on }, logger = console, { host }) {
    ...
    const openTestRunner = function (host) {
                ...
                return browser.newPage().then((page) => {
                    return page.setViewport({
                        ...
                        return browser.userAgent().then((agent) => {
                            console.log('agent: ' + browser.userAgent().then((agent) => { console.log(agent) }));
                        ...
                        if (!process.env.CICD_ATF_TEST_USER_TOKEN) { // get a valid cookie
                            return page.goto(`${host}/login.do?user_name=${username}&sys_action=sysverb_login&user_password=${password}`, {
                                ...
                                console.log("Second: " +Json.stringify(response.headers()));
                            });
                        }
                    }).then(() => {
                        return page.goto(`${host}/atf_test_runner.do?sysparm_nostack=true`, {
                            ...
                            console.log("Third: " +Json.stringify(response.headers()));
                        }).catch((error) => {
        ...
        }).delay(50000).then(({ browser, runnerId }) => {
    ...
    return Promise.try(() => {
        ...
        return Promise.try(() => {
            ...
            if (!normalBuildRun) {
                return slack.message(`ATF Run. Going to execute ${test.suites.length > 0 ? `${test.suites.length} suite${test.suites.length > 1 ? 's' : ''}` : 'NO suite'} and ${test.tests.length > 0 ? `${test.tests.length} test${test.tests.length > 1 ? 's' : ''}` : 'NO tests'} for update set <${config.host.name}/sys_update_set.do?sys_id=${config.updateSet.sys_id}|${config.updateSet.name}> on <${config.host.name}|${config.host.name}>`);
            }
        }).then(() => {
        ...
        return self.db.test.update(test).then(() => {
            if (!normalBuildRun) {
                const text = `Test ${test.passed ? 'PASSED' : 'FAILED'} for update set <${config.host.name}/sys_update_set.do?sys_id=${config.updateSet.sys_id}|${config.updateSet.name}>\n on <${config.host.name}|${config.host.name}>. Please check the <${config.application.docUri}|test results>`;
                if (test.passed)
